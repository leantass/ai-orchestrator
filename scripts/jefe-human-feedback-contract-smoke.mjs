...
const feedback = buildHumanFeedback({ approval: { ...approval, artifactSha256: 'artifact-feedback', correctionId: 'correction-feedback', returnTarget: 'execution', authority: 'human_decision', actor: { type: 'human', identity: 'human-reviewer' } }, preview: { versionSnapshot: { artifactSha256: 'artifact-feedback' } }, projectBrief: { objective: 'Ordenar la propuesta.' }, businessUnderstanding: { audience: 'Equipos comerciales' }, qualityReports: { content: { status: 'needs_revision' } } })
...
assert.equal(feedback.artifactSha256, 'artifact-feedback')
assert.equal(feedback.correctionId, 'correction-feedback')
assert.equal(feedback.returnTarget, 'execution')
