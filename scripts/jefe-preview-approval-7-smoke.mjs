...
try {
  ...
  const structuredReview = await service.review({ projectId: input.projectId, previewRequestId: preview.previewRequestId, decision: 'pending', reviewType: 'visual', findings: ['  Hallazgo   visual   controlado  '], questions: ['pregunta de smoke'] })
  check(structuredReview.reviewType === 'visual' && structuredReview.findings[0] === 'Hallazgo visual controlado' && structuredReview.questions.length === 1, '7B-04 reviewType/findings/questions persistidos y normalizados')
