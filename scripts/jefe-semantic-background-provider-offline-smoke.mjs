...
const deadlineResult = await deadline.provider.request({ executionMode: 'background', input: [], maxOutputTokens: 1200 }); assert.equal(deadlineResult.errorCategory, 'BACKGROUND_POLL_LIMIT_EXCEEDED'); assert.equal(deadlineResult.telemetry.terminationReason, 'POLL_LIMIT'); assert.equal(deadline.counters().cancel, 1)
...
console.log('PASS jefe-semantic-background-provider-offline-smoke: completed/incomplete-retry/failed/cancelled/poll-limit-cancel/transient-poll, store=false, bounded polling, generation budget separation, transport stats')
