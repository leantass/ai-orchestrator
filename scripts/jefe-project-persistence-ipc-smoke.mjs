...
try {
  ...
  assert.equal(projects[0].planning.brief.projectName, 'Persistencia')
  assert.equal((await persistence.listVersions('persist-project')).length, 1)
  const snapshot = await persistence.workspaceSnapshot('persist-project')
  assert.equal(snapshot.previewAvailable, true)
  assert.equal(snapshot.projectName, 'Persistencia')
