...
const { resolveSemanticCopyOwnership, compareGeneratedContent } = require('../electron/jefe-product-planning.cjs')
...
assert.equal(resolveSemanticCopyOwnership(planning).presentationOwner, 'presentation')
const regeneratedRoot = path.join(root, 'regenerated')
const regeneratedProject = { ...source.project, planning, activeVersionId: 'version-regenerated', runId: 'run-regenerated', physicalPaths: { projectRoot: path.join(regeneratedRoot, source.project.projectId, 'version-regenerated'), manifestPath: path.join(regeneratedRoot, source.project.projectId, 'version-regenerated', 'manifest.json'), deliveryPath: null }, versions: [{ ...source.project.versions[0], versionId: 'version-regenerated', runId: 'run-regenerated' }] }
await materializeProject({ project: regeneratedProject, destinationRoot: regeneratedRoot, capabilities: {}, profileContext: { generationMode: 'semantic_correction', semanticGenerationSpec: { schemaVersion: 'semantic-generation-spec-v1', planning, sectionOrder: planning.experience.sections, heroVariant: 'focused', treatments: ['semantic'] } }, providedAssets: [] })
const regeneratedHtml = await fs.readFile(path.join(regeneratedRoot, source.project.projectId, 'version-regenerated', 'app/index.html'), 'utf8')
const presentationPayload = planning.content.presentation
assert.equal((regeneratedHtml.match(new RegExp(presentationPayload.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&'), 'gu')) || []).length, 1)
assert.doesNotMatch(regeneratedHtml, /class="lede"[^>]*>[^<]*Trabajo con personas/iu)
assert.equal(compareGeneratedContent(planning, regeneratedHtml).pass, true)
assert.equal(assessCrossSectionRepetition({ html: regeneratedHtml }).status, 'PASS')
const omittedPresentation = adaptSemanticPlansToPlanning({ sourcePlanning: source.project.planning, businessUnderstanding: bu, contentPlan: { ...content, sections: content.sections.filter((item) => item.contentRef !== 'presentation') }, experiencePlan: { ...experience, sectionOrder: experience.sectionOrder.filter((id) => id !== 'narrativa-clara') } })
assert.equal(resolveSemanticCopyOwnership(omittedPresentation).presentationOwner, 'hero')
