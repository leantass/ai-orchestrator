...
const specFor = (content, experience) => ({ schemaVersion: 'semantic-generation-spec-v1', ...{ planning: adaptSemanticPlansToPlanning({ sourcePlanning: source.project.planning, businessUnderstanding: bu, contentPlan: content, experiencePlan: experience }), sectionOrder: experience.sectionOrder, heroVariant: experience.heroVariant, treatments: experience.sectionTreatments, creativeDirection: 'editorial', contentDensity: experience.contentDensity, ctaStrategy: experience.conversionStrategy, preservedQualities: ['responsive'], prohibitedChanges: ['source-version-immutable'] } })
...
assert.equal(specA.planning.semanticCtaResolution.canonicalPrimaryCta, 'Solicitar una reunión')
assert.equal(specA.planning.semanticCtaResolution.providerProposedCta, 'Consultar A.')
assert.equal(specA.planning.semanticCtaResolution.primaryCtaOverrideApplied, true)
...
assert.match(htmlA, /Hero semántico A/u); assert.match(htmlA, /Servicio A/u); assert.match(htmlA, /<a class="cta"[^>]*>Solicitar una reunión<\/a>/u)
assert.match(htmlB, /Hero semántico B/u); assert.match(htmlB, /Servicio C/u); assert.match(htmlB, /<a class="cta"[^>]*>Solicitar una reunión<\/a>/u)
assert.match(htmlA, /Presentación semántica A/u); assert.match(htmlA, /visible/u)
...
assert.deepEqual(adaptSemanticGenerationSpec(labelledSpec).sectionOrder, ['inicio', 'servicios', 'confianza', 'faq', 'contacto'])
