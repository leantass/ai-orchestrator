...
try {
  ...
  assert.deepEqual(Object.keys(CHANNELS).sort(), ['capabilities', 'correction', 'corrections', 'findings', 'gates', 'recovery', 'request', 'runs', 'snapshot'].sort())
  ...
  const runs = await handlers.get(CHANNELS.runs)(null, { projectId: 'ipc-project' })
  assert.deepEqual(runs.runs.map((item) => item.qaRunId), [requested.run.qaRunId])
