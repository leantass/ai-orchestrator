...
const { SEMANTIC_SECTION_CONTRACTS, validateCanonicalSemanticContentCatalog } = require('./jefe-semantic-generation-adapter.cjs')
...
const TRUST_ITEM_SCHEMA = Object.freeze({ type: 'object', additionalProperties: false, properties: { title: { type: 'string' }, description: { type: 'string' } }, required: ['title', 'description'] })
const SERVICE_ITEM_SCHEMA = Object.freeze({ type: 'object', additionalProperties: false, properties: { title: { type: 'string' }, description: { type: 'string' } }, required: ['title', 'description'] })
const CONTACT_SCHEMA = Object.freeze({ type: 'object', additionalProperties: false, properties: { ctaLabel: { type: 'string' }, supportingText: { type: 'string' } }, required: ['ctaLabel', 'supportingText'] })
const CONTENT_PLAN_SCHEMA = Object.freeze({ name: 'content_plan_v2', schema: { type: 'object', additionalProperties: false, properties: { schemaVersion: { type: 'string', enum: ['content-plan-v2'] }, hero: { type: 'string' }, presentation: { type: 'string' }, services: { type: 'array', items: SERVICE_ITEM_SCHEMA, minItems: 1, maxItems: 8 }, trust: { type: 'array', items: TRUST_ITEM_SCHEMA, minItems: 1, maxItems: 6 }, faq: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { question: { type: 'string' }, answer: { type: 'string' } }, required: ['question', 'answer'] }, minItems: 4, maxItems: 8 }, contact: CONTACT_SCHEMA, contentPriorities: { type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 8 }, sections: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { id: { type: 'string', pattern: '^[a-z][a-z0-9-]{0,31}$' }, role: { type: 'string', enum: [...new Set(Object.values(SEMANTIC_SECTION_CONTRACTS).map((contract) => contract.role))] }, label: { type: 'string' }, kind: { type: 'string', enum: [...new Set(Object.values(SEMANTIC_SECTION_CONTRACTS).map((contract) => contract.kind))] }, required: { type: 'boolean' }, contentRef: { type: 'string', enum: Object.keys(SEMANTIC_SECTION_CONTRACTS) }, aliases: { type: 'array', items: { type: 'string' } } }, required: ['id', 'role', 'label', 'kind', 'required', 'contentRef', 'aliases'] }, minItems: 1, maxItems: 8 } }, required: ['schemaVersion', 'hero', 'presentation', 'services', 'trust', 'faq', 'contact', 'contentPriorities', 'sections'] } })
...
function validateContentPlanDecision(value) {
  ...
  const failContent = (code, message, field) => { throw Object.assign(new Error(message), { code, details: { field } }) }
  const services = value?.services
  const placeholder = /lorem ipsum|placeholder|\[\s*(?:texto|completar|todo)|\b(?:tbd|n\/a)\b/iu
  const genericServiceTitle = /^(?:pack|paquete|servicio|plan|opci[oó]n)\s*\d+\s*$/iu
  if (!Array.isArray(services) || services.length < 1 || services.length > 8 || services.some((item) => !item || typeof item !== 'object' || Array.isArray(item) || Object.keys(item).sort().join('|') !== 'description|title' || typeof item.title !== 'string' || !item.title.trim() || typeof item.description !== 'string' || !item.description.trim())) failContent('SEMANTIC_CONTENT_PLAN_SERVICES_INVALID', 'ContentPlanV2 services debe contener objetos title/description separados.', 'services')
  for (const item of services) {
    const title = item.title.trim(); const description = item.description.trim()
    if (placeholder.test(`${title} ${description}`)) failContent('SEMANTIC_CONTENT_PLAN_SERVICE_INVALID', 'ContentPlanV2 service no puede contener placeholders.', 'services')
    if (genericServiceTitle.test(title) || title.length > 80 || title.split(/\s+/u).length > 12) failContent('SEMANTIC_CONTENT_PLAN_SERVICE_TITLE_INVALID', 'Cada service debe tener un título corto y significativo.', 'services.title')
    const normalizedTitle = title.replace(/[.!?]+$/u, '').toLocaleLowerCase('es-AR').normalize('NFD').replace(/[\u0300-\u036f]/gu, '').replace(/\s+/gu, ' ')
    const normalizedDescription = description.replace(/[.!?]+$/u, '').toLocaleLowerCase('es-AR').normalize('NFD').replace(/[\u0300-\u036f]/gu, '').replace(/\s+/gu, ' ')
    if (normalizedTitle === normalizedDescription) failContent('SEMANTIC_CONTENT_PLAN_SERVICE_DUPLICATE', 'El título y la descripción del service deben ser distintos.', 'services')
  }
  const trust = value?.trust
  if (!Array.isArray(trust) || trust.length < 1 || trust.length > 6 || trust.some((item) => !item || typeof item !== 'object' || Array.isArray(item) || Object.keys(item).sort().join('|') !== 'description|title' || typeof item.title !== 'string' || !item.title.trim() || typeof item.description !== 'string' || !item.description.trim())) failContent('SEMANTIC_CONTENT_PLAN_TRUST_INVALID', 'ContentPlanV2 trust debe contener títulos y descripciones concretos.', 'trust')
  if (trust.some((item) => /^(?:criterio|raz[oó]n|punto|item|confianza|calidad)\s*\d*$/iu.test(item.title.trim()))) failContent('SEMANTIC_CONTENT_PLAN_TRUST_TITLE_INVALID', 'ContentPlanV2 trust no puede usar títulos genéricos.', 'trust.title')
  const contact = value?.contact
  if (!contact || typeof contact !== 'object' || Array.isArray(contact) || Object.keys(contact).sort().join('|') !== 'ctaLabel|supportingText' || typeof contact.ctaLabel !== 'string' || !contact.ctaLabel.trim() || typeof contact.supportingText !== 'string' || !contact.supportingText.trim()) failContent('SEMANTIC_CONTENT_PLAN_CONTACT_INVALID', 'ContentPlanV2 contact debe separar CTA y supportingText.', 'contact')
  const ctaWords = contact.ctaLabel.trim().split(/\s+/u)
  if (contact.ctaLabel.length > 72 || ctaWords.length > 9 || /lorem ipsum|placeholder|\[\s*(?:texto|completar|todo)|\b(?:tbd|n\/a)\b|\b\d+[.)]\s|\b(?:agend|escrib|llam)[aáeéiíoóuú]?\b[\s\S]*\b(?:agend|escrib|llam)[aáeéiíoóuú]?\b/iu.test(contact.ctaLabel)) failContent('SEMANTIC_CONTENT_PLAN_CTA_INVALID', 'ContentPlanV2 ctaLabel debe ser una acción breve.', 'contact.ctaLabel')
  const hero = String(value?.hero || '').trim()
  const heroSentences = hero.split(/(?<=[.!?])\s+/u)
  if (/\b[\w.+-]+@[\w.-]+\.[a-z]{2,}\b|https?:\/\/|www\.|\+?\d[\d\s().-]{7,}\d/iu.test(hero) || /\b(?:1[.)]|2[.)]|3[.)])\s/u.test(hero) || heroSentences.slice(1).some((sentence) => /\b(?:agend|escrib|llam|contact)[aáeéiíoóuú]?\b/iu.test(sentence))) failContent('SEMANTIC_CONTENT_PLAN_HERO_INVALID', 'ContentPlanV2 hero debe ser una propuesta de valor sin datos ni acciones de contacto.', 'hero')
  try { validateCanonicalSemanticContentCatalog(value?.sections) } catch (error) { throw error }
  return value
}
