...
function resolveFactoryHermesControlledResearchRuntimeAlternateSafeRuntimeImplementationPlanningPaths(root = process.cwd()) {
  const installRoot = path.join(root, '.codex-temp', 'external-tools', 'hermes-agent', 'install', '75b300f');
  return {
    repoRoot: root,
    installRoot,
    resultArtifact: path.join(installRoot, 'controlled-research-runtime-alternate-safe-runtime-implementation-planning-result.json'),
...
function assertControlledResearchRuntimeAlternateSafeRuntimeImplementationPlanningPathContained(target, root) {
