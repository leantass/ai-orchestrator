...
function resolveFactoryHermesControlledResearchRuntimePlanningPaths(root = process.cwd()) {
  ...
  return {
    ...
    planningResult: path.join(installRoot, 'controlled-research-runtime-planning-result.json'),
  };
}

function assertControlledResearchRuntimePlanningPathContained(target, root) {
  const rel = path.relative(path.resolve(root), path.resolve(target));
  if (rel.startsWith('..') || path.isAbsolute(rel)) throw new Error(`Path escapes allowed root: ${target}`);
}

module.exports = { resolveFactoryHermesControlledResearchRuntimePlanningPaths, assertControlledResearchRuntimePlanningPathContained };
