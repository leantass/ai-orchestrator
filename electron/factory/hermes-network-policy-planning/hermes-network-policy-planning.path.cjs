...
function resolveFactoryHermesNetworkPolicyPlanningPaths() {
...
function assertNetworkPolicyPlanningPathContained(target, root) {
  const rel = path.relative(root, target);
  if (rel.startsWith('..') || path.isAbsolute(rel)) throw new Error(`path_outside_root: ${target}`);
}

module.exports = { resolveFactoryHermesNetworkPolicyPlanningPaths, assertNetworkPolicyPlanningPathContained };
