...
function validateExecutionContract(value) {
  if (!plain(value) || value.schemaVersion !== SCHEMA_VERSION || !EXECUTION_ID.test(value.executionId) || !ATTEMPT_ID.test(value.attemptId) || !Number.isSafeInteger(value.attemptNumber) || value.attemptNumber < 1) fail('INVALID_EXECUTION_CONTRACT', 'Contrato de ejecucion invalido.')
  if (value.attemptId !== `execution-attempt-${digest({ executionId: value.executionId, attemptNumber: value.attemptNumber }).slice(0, 32)}`) fail('INVALID_EXECUTION_CONTRACT', 'Intento no correlacionado.')
