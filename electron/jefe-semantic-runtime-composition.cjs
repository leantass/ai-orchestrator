...
function createSemanticRuntimeComposition({ root, feedbackProvider = null, decisionProvider = null, semanticProvider = null, semanticBrainAdapter = null, mode = 'synthetic', env = process.env, callBudget = null } = {}) {
  ...
  async function runSemanticPlans({ brief, correctionPlan = null, feedback = null, preservedQualities = [], sourceRefs = ['synthetic-brief:composition'], onPhase = async () => {}, onPhaseStart = () => {} } = {}) {
    ...
    const content = await decideSemantic({ operation: 'content_plan', input: [{ role: 'system', content: [{ type: 'input_text', text: 'ContentPlanV2 debe producir copy customer-facing. Hero: sólo propuesta de valor, sin email, teléfono ni CTA. Trust: objetos title y description concretos. Contact: objeto ctaLabel breve y supportingText separado; nunca uses un párrafo como botón. FAQ: preguntas y respuestas separadas; el heading FAQ no se decide desde una pregunta. No inventes contentRef por paquete ni uses instrucciones internas de diseño.' }] }, { role: 'user', content: [{ type: 'input_text', text: JSON.stringify({ brief, businessUnderstanding: bu.decision, correctionPlan, preservedQualities }) }] }], schema: CONTENT_PLAN_SCHEMA, sourceRefs, outputBudgetRetryMax: 1, executionMode: 'background' })
