...
const { adaptSemanticPlansToPlanning, buildContentSectionCatalog, contentSectionCatalogHash, resolveCanonicalPrimaryCta } = require('./jefe-semantic-generation-adapter.cjs')
...
function createSemanticRuntimeComposition({ root, feedbackProvider = null, decisionProvider = null, semanticProvider = null, semanticBrainAdapter = null, mode = 'synthetic', env = process.env, callBudget = null } = {}) {
  ...
  const resolveExecution = async ({ projectId, sourceVersionId }, { onPhase = async () => {}, onPhaseStart = () => {} } = {}) => {
    ...
    const canonicalPrimaryCta = resolveCanonicalPrimaryCta(source.project.planning)
    const realPlans = productive ? await runSemanticPlans({ brief: { ...brief, ...(canonicalPrimaryCta ? { primaryCta: canonicalPrimaryCta } : {}) }, feedback, protectedHumanFields: { primaryCta: canonicalPrimaryCta }, sourceRefs: [`semantic-source:${projectId}:${sourceVersionId}`], onPhase, onPhaseStart }) : null
    ...
    if (!realPlans) onPhaseStart('CORRECTION_PLAN_READY')
    const correctionPlan = realPlans?.correctionPlan || intelligence.createCorrectionPlan({ feedback, businessUnderstanding: understanding })
    if (!realPlans) await onPhase('CORRECTION_PLAN_READY')
    ...
    const semanticPlanning = productive ? adaptSemanticPlansToPlanning({ sourcePlanning: source.project.planning, businessUnderstanding: understanding, contentPlan, experiencePlan, canonicalPrimaryCta, requireCatalogHash: true }) : source.project.planning
  ...
  async function runSemanticPlans({ brief, correctionPlan = null, feedback = null, protectedHumanFields = {}, preservedQualities = [], sourceRefs = ['synthetic-brief:composition'], onPhase = async () => {}, onPhaseStart = () => {} } = {}) {
    requireProductiveProvider()
    ...
    const localCorrectionPlan = correctionPlan || (feedback ? intelligence.createCorrectionPlan({ feedback, businessUnderstanding: bu.decision }) : null)
    await onPhase('CORRECTION_PLAN_READY')
    const contentInput = { brief, businessUnderstanding: bu.decision, correctionPlan: localCorrectionPlan, feedback, protectedHumanFields, preservedQualities }
    const content = await decideSemantic({ operation: 'content_plan', input: [{ role: 'system', content: [{ type: 'input_text', text: 'ContentPlanV2 debe producir copy customer-facing. El hero.title debe ser un heading escaneable: una sola propuesta clara, máximo 160 caracteres y 24 palabras; nunca un párrafo ni varias ideas encadenadas. primaryCta es human-owned y protectedHumanFields.primaryCta debe conservarse exactamente; no parafrasear ni reemplazar. No inventes afirmaciones factuales: experiencia, credenciales, clientes, resultados garantizados, plazos, SLA, porcentajes, métricas, metodologías, herramientas, tecnologías, precios, disponibilidad o tiempos de respuesta sólo pueden aparecer si están explícitamente respaldados por brief, objective, proposition, services, materials o feedback. Si no hay evidencia, usa copy general y prudente.' }] }, { role: 'user', content: [{ type: 'input_text', text: JSON.stringify(contentInput) }] }], schema: CONTENT_PLAN_SCHEMA, sourceRefs, outputBudgetRetryMax: 1, executionMode: 'background' })
    ...
    onPhaseStart('EXPERIENCE_PLAN_READY')
    const experience = await decideSemantic({ operation: 'experience_plan', input: [{ role: 'user', content: [{ type: 'input_text', text: JSON.stringify({ brief, businessUnderstanding: bu.decision, contentPlan: content.decision, contentSectionCatalog, contentSectionCatalogHash: catalogHash, correctionPlan: localCorrectionPlan, protectedHumanFields, preservedQualities }) }] }], schema: experiencePlanSchemaForCatalog(contentSectionCatalog, catalogHash), sourceRefs, contentPlanRef: 'ContentPlanV2', contentSectionCatalogHash: catalogHash, outputBudgetRetryMax: 1, executionMode: 'background' })
    await onPhase('EXPERIENCE_PLAN_READY', { semanticGenerationCalls: 3 })
    return { businessUnderstanding: bu, correctionPlan: localCorrectionPlan, contentPlan: content, experiencePlan: experience, providerBudget: runBudget?.snapshot?.() || null }
  }
