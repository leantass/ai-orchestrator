...
function commercialLayoutV2(direction, brandName, action, logoAppPath = null, planning = null) {
  ...
  const renderSemanticTrust = (section) => `<section id="${anchor(section)}" class="trust-grid"><div class="section-heading"><p class="eyebrow">${escapeHtml(label(section) || 'Confianza')}</p><h2>${escapeHtml(label(section) || 'Confianza')}</h2></div>${trust}</section>`
  const renderSemanticFaq = (section) => `<section id="${anchor(section)}" class="faq-panel"><div class="section-heading"><p class="eyebrow">${escapeHtml(label(section) || 'Preguntas frecuentes')}</p><h2>${escapeHtml(label(section) || 'Preguntas frecuentes')}</h2></div>${faq}</section>`
  const renderSemanticServices = (section) => `<section id="${anchor(section)}" class="benefit-grid"><div class="section-heading"><p class="eyebrow">${escapeHtml(label(section) || 'Servicios')}</p><h2>${escapeHtml(label(section) || 'Servicios')}</h2></div>${benefits}</section>`
  const renderSemanticContact = (section) => `<section id="${anchor(section)}" class="conversion-panel"><p class="eyebrow">${escapeHtml(label(section) || 'Próximo paso')}</p><h2>${escapeHtml(content.contact?.title || action)}</h2><p class="contact-supporting">${escapeHtml(content.contact?.description || '')}</p>${form}</section>`
  const renderFaqSection = (section) => `<section id="${anchor(section)}" class="faq-panel"><div class="section-heading"><p class="eyebrow">${escapeHtml(label(section) || 'Preguntas frecuentes')}</p><h2>${escapeHtml(label(section) || 'Preguntas frecuentes')}</h2></div>${faq}</section>`
  const renderSection = (section) => {
    const type = sectionType(section)
    if (type === 'faq') return renderFaqSection(section)
    if (!semanticMode && type === 'hero') return renderLegacyHeroClean(section)
    ...
    if (semanticMode && type === 'trust') return renderSemanticTrust(section)
    if (semanticMode && type === 'services') return renderSemanticServices(section)
    if (semanticMode && type === 'contact') return renderSemanticContact(section)
