...
function registerCanonicalProjectIpc({ ipcMain, root, shell, clipboard, contextMemory = null, createFirstVersion = createFirstVersionFromRun, semanticRuntimeAdapter = null }) {
  ...
  handle(CHANNELS.semanticCorrection, async (payload) => { noPathPayload(payload); if (!semanticRuntimeAdapter || typeof semanticRuntimeAdapter.requestSemanticCorrection !== 'function') { const error = new Error('El runtime semántico no está configurado.'); error.code = 'SEMANTIC_RUNTIME_NOT_CONFIGURED'; throw error }; const projectId = id(payload.projectId, 'projectId'); const sourceVersionId = id(payload.sourceVersionId, 'sourceVersionId'); return semanticRuntimeAdapter.requestSemanticCorrection({ projectId, sourceVersionId, ...(payload.idempotencyKey ? { idempotencyKey: payload.idempotencyKey } : {}) }) })
