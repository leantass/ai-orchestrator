...
function createSemanticRuntimeAdapter({ root, service, resolveExecution, timeoutMs = DEFAULT_TIMEOUT_MS, providerOperationTimeoutMs = null, now = Date.now } = {}) {
  ...
  async function requestSemanticCorrection(input) {
    ...
    try {
      ...
      settled = true
      const providerTimeout = error?.errorEnvelope?.category === 'BACKGROUND_DEADLINE_EXCEEDED'
      const timeoutDetails = error?.timeoutDetails || (providerTimeout ? { timeoutLayer: 'BACKGROUND_DEADLINE', operation: error.errorEnvelope.operation || null, elapsedMs: error.telemetry?.durationMs || null, generationCalls: error.errorEnvelope.generationCallNumber || state.semanticGenerationCalls, pollCount: error.errorEnvelope.pollCount || error.telemetry?.pollRequests || 0 } : null)
      const category = providerTimeout ? 'BACKGROUND_DEADLINE' : failureCategory(error, activePhase)
      state = await stateStore.terminal(state, 'BLOCKED', { failureCategory: category, safeFailureMessage: safeFailureMessage(error), lastCompletedPhase: state.lastCompletedPhase, candidateGenerationDetails: safeCandidateGenerationDetails(error), timeoutDetails, qualityDetails: safeQualityDetails(error) })
