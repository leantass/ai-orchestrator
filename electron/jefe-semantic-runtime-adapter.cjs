...
function safeQualityDetails(error) { const report = error?.details?.report || error?.details?.qualityReport || error?.report; if (!report || typeof report !== 'object') return null; const gateNames = ['semanticPreGate', 'semanticPlanFidelity', 'artifactIndependence', 'visualQuality', 'contentQuality', 'experienceQuality', 'browserQuality']; const safeGate = (gate) => ({ status: typeof gate?.status === 'string' ? gate.status : gate?.pass === true ? 'PASS' : gate?.pass === false ? 'NEEDS_CORRECTION' : null, findingCount: Number.isInteger(gate?.findingCount) ? gate.findingCount : Array.isArray(gate?.findings) ? gate.findings.length : Array.isArray(gate?.report?.findings) ? gate.report.findings.length : 0 }); const safeFinding = (finding, gate = null) => ({ gate, category: typeof finding?.category === 'string' ? finding.category.slice(0, 120) : null, selector: typeof finding?.selector === 'string' ? finding.selector.slice(0, 200) : null, expected: typeof finding?.expected === 'string' ? finding.expected.slice(0, 300) : null, actual: typeof finding?.actual === 'string' ? finding.actual.slice(0, 300) : null, message: typeof finding?.message === 'string' ? finding.message.slice(0, 300) : null }); const findings = Array.isArray(report.findings) ? report.findings.slice(0, 100).map((finding) => safeFinding(finding, null)) : []; for (const gateName of gateNames) { const gate = report[gateName]; const source = Array.isArray(gate?.findings) ? gate.findings : Array.isArray(gate?.report?.findings) ? gate.report.findings : []; findings.push(...source.slice(0, 100).map((finding) => safeFinding(finding, gateName))); } return { overallStatus: report.overallStatus || null, eligibleForPromotion: report.eligibleForPromotion === true, gates: Object.fromEntries(gateNames.map((gateName) => [gateName, safeGate(report[gateName])])), activeSectionIds: Array.isArray(report.activeSectionIds) ? report.activeSectionIds.slice(0, 100).filter((item) => typeof item === 'string').map((item) => item.slice(0, 80)) : [], activeContentRefs: Array.isArray(report.activeContentRefs) ? report.activeContentRefs.slice(0, 100).filter((item) => typeof item === 'string').map((item) => item.slice(0, 80)) : [], requiredContentRefs: Array.isArray(report.requiredContentRefs) ? report.requiredContentRefs.slice(0, 100).filter((item) => typeof item === 'string').map((item) => item.slice(0, 80)) : [], omittedOptionalContentRefs: Array.isArray(report.omittedOptionalContentRefs) ? report.omittedOptionalContentRefs.slice(0, 100).filter((item) => typeof item === 'string').map((item) => item.slice(0, 80)) : [], browserChecks: report.browserChecks && typeof report.browserChecks === 'object' ? Object.fromEntries(Object.entries(report.browserChecks).slice(0, 40).map(([key, value]) => [key.slice(0, 80), typeof value === 'boolean' || typeof value === 'number' || typeof value === 'string' ? value : null])) : {}, overallContentStatus: report.overallContentStatus || null, findings: findings.slice(0, 100) } }
