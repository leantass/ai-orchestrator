...
function semanticContact(value, fallback = '') {
  if (value && typeof value === 'object' && !Array.isArray(value)) return { ctaLabel: sentence(value.ctaLabel, fallback), supportingText: bodySentence(value.supportingText, fallback), legacy: false }
  const exact = sentence(value, fallback)
  return { ctaLabel: exact, supportingText: bodySentence(exact, fallback), legacy: true }
}
