...
const { assessArtifact, assessContentQuality, assessCrossSectionRepetition } = require('./jefe-generator-quality.cjs')
...
function aggregate(reports) { const required = ['semanticPreGate', 'semanticPlanFidelity', 'artifactIndependence', 'visualQuality', 'contentQuality', 'crossSectionRepetition', 'experienceQuality', 'browserQuality']; const pass = required.every((key) => reports[key]?.pass === true || reports[key]?.status === 'PASS'); return { ...reports, overallStatus: pass ? 'PASS' : 'NEEDS_CORRECTION', eligibleForPromotion: pass } }
