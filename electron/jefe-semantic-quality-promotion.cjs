...
const { assessArtifact, assessContentQuality, assessCrossSectionRepetition, assessCustomerFacingCopy } = require('./jefe-generator-quality.cjs')
const { compareGeneratedContent, resolveArtifactContentSelection } = require('./jefe-product-planning.cjs')
...
function aggregate(reports) { const required = ['semanticPreGate', 'semanticPlanFidelity', 'artifactIndependence', 'visualQuality', 'contentQuality', 'crossSectionRepetition', 'heroQuality', 'ctaQuality', 'trustHeadingQuality', 'faqHeadingQuality', 'customerFacingLanguage', 'experienceQuality', 'browserQuality']; const pass = required.every((key) => reports[key]?.pass === true || reports[key]?.status === 'PASS'); return { ...reports, overallStatus: pass ? 'PASS' : 'NEEDS_CORRECTION', eligibleForPromotion: pass } }
