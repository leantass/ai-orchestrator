...
function aggregate(reports) { const required = ['semanticPreGate', 'semanticPlanFidelity', 'artifactIndependence', 'visualQuality', 'contentQuality', 'crossSectionRepetition', 'serviceCardQuality', 'renderedServiceCardQuality', 'heroQuality', 'ctaQuality', 'trustHeadingQuality', 'faqHeadingQuality', 'customerFacingLanguage', 'primaryCtaFidelity', 'renderedPrimaryCtaFidelity', 'experienceQuality', 'browserQuality']; const pass = required.every((key) => reports[key]?.pass === true || reports[key]?.status === 'PASS'); return { ...reports, overallStatus: pass ? 'PASS' : 'NEEDS_CORRECTION', eligibleForPromotion: pass } }
