...
function aggregate(reports) { const required = ['semanticPreGate', 'semanticPlanFidelity', 'artifactIndependence', 'visualQuality', 'contentQuality', 'experienceQuality', 'browserQuality']; const pass = required.every((key) => reports[key]?.pass === true || reports[key]?.status === 'PASS'); return { ...reports, overallStatus: pass ? 'PASS' : 'NEEDS_CORRECTION', eligibleForPromotion: pass } }
