...
function visibleEntries(row, definition) { const preferred = Object.keys(definition.fields || {}).filter((field) => Object.prototype.hasOwnProperty.call(row, field)); const fields = preferred.length > 0 ? preferred : Object.keys(row); return fields.filter((field) => field !== 'id' && row[field] !== undefined && row[field] !== null && row[field] !== '').slice(0, 8).map((field) => [field, row[field]]) }
...
async function renderSummary() { const dashboard = await api('/api/dashboard').then((result) => result.body); summary.innerHTML = Object.entries(dashboard.summary || {}).map(([collection, count]) => '<article class="metric-card"><strong>' + escapeHtml(count) + '</strong><span>' + escapeHtml(labelFor(collection)) + '</span></article>').join('') }
...
async function renderRows() {
  ...
  const rowsHtml = rows.length > 0 ? '<div class="data-grid">' + rows.map((row) => renderRecordCard(row, definition, true)).join('') + '</div>' : renderEmpty(definition.label)
  list.innerHTML = renderCollectionTabs() + '<div class="section-heading"><p>' + rows.length + ' registros activos</p><h3>' + escapeHtml(definition.label) + '</h3></div>' + rowsHtml
...
function renderStatusBadge(row) { const status = statusForRow(row); return status ? '<span class="status-badge">' + escapeHtml(humanize(status)) + '</span>' : '' }
function renderRecordFields(row, definition) { const entries = visibleEntries(row, definition); if (entries.length === 0) return '<p class="muted">Sin campos visibles.</p>'; return '<dl class="record-fields">' + entries.map(([field, value]) => '<div><dt>' + escapeHtml(humanize(field)) + '</dt><dd>' + escapeHtml(formatValue(field, value)) + '</dd></div>').join('') + '</dl>' }
function renderRecordCard(collection, row, definition) { return '<article class="record-card' + semanticClass(collection) + '"><div class="record-card-header"><strong>' + escapeHtml(titleForRow(row)) + '</strong>' + renderStatusBadge(row) + '</div>' + renderRecordFields(row, definition) + '</article>' }
...
async function renderSummary() { const dashboard = await api('/api/dashboard').then((result) => result.body); const allowed = new Set(Object.values(page?.collections || {})); const entries = Object.entries(dashboard.summary || {}).filter(([collection]) => allowed.size === 0 || allowed.has(collection)); summary.innerHTML = entries.map(([collection, count]) => '<article class="metric-card"><strong>' + escapeHtml(count) + '</strong><span>' + escapeHtml(labelFor(collection)) + '</span></article>').join('') }
...
function buildCss() {
  return `:root{font-family:"Trebuchet MS",Verdana,sans-serif;color:#14231d;background:#f7f4ea;--ink:#102820;--ink-2:#173d32;--paper:#fffdf7;--line:#d8ded4;--soft:#e7f2df;--accent:#f1c27d;--blue:#dfeaf7;--danger:#8a2f2b}*{box-sizing:border-box}html,body{width:100%;min-width:100%;margin:0}body{background:linear-gradient(120deg,#f7f4ea 0%,#f7f4ea 55%,#edf4ec 100%)}.topbar{width:100%;display:flex;justify-content:space-between;align-items:center;gap:18px;padding:18px 32px;background:var(--ink);color:white}.topbar strong{font-size:1.05rem}.topbar nav{display:flex;gap:10px;flex-wrap:wrap}.topbar a{color:white;text-decoration:none;border:1px solid rgba(255,255,255,.22);border-radius:999px;padding:7px 10px}.layout{width:calc(100% - 48px);max-width:1360px;margin:0 auto;padding:28px 0 48px;display:grid;grid-template-columns:minmax(0,1fr);gap:22px}.hero{background:linear-gradient(135deg,var(--accent),#f8dfac);padding:32px;border-radius:8px;border:1px solid rgba(16,40,32,.12);box-shadow:0 18px 42px rgba(16,40,32,.08)}.role-layout .hero{background:linear-gradient(135deg,var(--soft),#f8fbf8)}.hero h1{max-width:900px;font-size:clamp(2.3rem,5vw,4.7rem);line-height:.95;margin:.12em 0 .24em}.hero p:not(.eyebrow){max-width:760px;font-size:1.15rem}.eyebrow{text-transform:uppercase;font-weight:800;letter-spacing:.04em;margin:0 0 8px}.panel{background:var(--paper);border:1px solid var(--line);padding:24px;border-radius:8px;box-shadow:0 12px 30px rgba(16,40,32,.05)}.section-heading{display:flex;justify-content:space-between;align-items:flex-end;gap:14px;margin-bottom:16px}.section-heading p{margin:0;color:#607066;text-transform:uppercase;font-size:.82rem;font-weight:800;letter-spacing:.04em}.section-heading h2,.section-heading h3{margin:0}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(132px,1fr));gap:12px;margin-top:22px}.metric-card{background:var(--ink);color:white;border-radius:8px;padding:14px 16px;min-height:86px;display:grid;align-content:center;gap:3px}.metric-card strong{font-size:2rem;line-height:1}.metric-card span{display:block;font-size:.86rem;text-transform:uppercase;letter-spacing:.04em;line-height:1.25;color:#d9eee5;overflow-wrap:anywhere}.collection-tabs,.role-actions,.role-terms{display:flex;flex-wrap:wrap;gap:10px}.collection-tabs{margin-bottom:18px}.collection-tabs button{background:#edf4ef;color:var(--ink);border-color:#c7d6cd}.collection-tabs button.active{background:var(--ink);color:white}.role-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px}.workflow-card{border:1px solid #d4ded8;background:#f8fbf8;border-radius:8px;padding:18px}.workflow-card h3{margin-top:0}.role-actions{margin:18px 0}.role-terms span{background:#eef5f0;color:var(--ink);padding:9px 11px;border-radius:999px;border:1px solid #cddbd2}.data-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.role-data-grid{grid-template-columns:repeat(auto-fit,minmax(260px,1fr))}.record-card{background:white;border:1px solid var(--line);border-radius:8px;padding:16px;display:grid;gap:12px;min-width:0}.record-card-header{display:flex;justify-content:space-between;align-items:flex-start;gap:10px}.record-card-header strong{font-size:1.05rem;overflow-wrap:anywhere}.record-fields{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin:0}.record-fields div{min-width:0}.record-fields dt{font-size:.72rem;text-transform:uppercase;letter-spacing:.04em;color:#657268;font-weight:800}.record-fields dd{margin:3px 0 0;overflow-wrap:anywhere}.record-actions{display:flex;gap:8px;flex-wrap:wrap}.status-badge{background:#e7f2df;color:var(--ink);border:1px solid #b8d2bf;border-radius:999px;padding:5px 8px;font-size:.78rem;font-weight:800}.printable-label{background:linear-gradient(180deg,#fff,#fffaf0);border:1px dashed #9c8a5a}.report-card{background:#f8fbff;border-color:#c9d8e8}.production-card{border-left:5px solid var(--accent)}.order-card{border-left:5px solid var(--ink-2)}.empty-state,.notice{border:1px solid var(--line);border-radius:8px;padding:16px;background:#f8fbf8;display:grid;gap:4px}.notice.success{border-color:#abcdb7;background:#edf7ef}.notice.danger{border-color:#e2b8b3;background:#fff1ef}.muted{color:#66746a}.row{display:grid;grid-template-columns:minmax(150px,1fr) minmax(220px,2fr) auto;gap:10px;align-items:center;border-bottom:1px solid #eee;padding:10px 0}.row span{display:flex;gap:8px;justify-content:flex-end}.row small{overflow-wrap:anywhere}form{display:grid;gap:12px;max-width:760px}label{display:grid;gap:4px;font-weight:800}input,select,button{font:inherit;padding:10px 12px;border:1px solid #b8c2bb;border-radius:6px}button{background:var(--ink);color:white;cursor:pointer}button:hover{background:var(--ink-2)}pre{white-space:pre-wrap;background:transparent;color:inherit;padding:0;border-radius:0}@media(max-width:760px){.topbar{display:block;padding:18px 22px}.topbar nav{display:grid;grid-template-columns:1fr;margin-top:12px}.topbar a{display:block;width:100%;overflow-wrap:anywhere}.layout{width:calc(100% - 28px);max-width:720px;padding-top:20px}.hero,.panel{padding:20px}.hero h1{font-size:clamp(1.75rem,8vw,2.25rem);line-height:1.05;overflow-wrap:anywhere}.hero p:not(.eyebrow){font-size:.92rem;line-height:1.35;overflow-wrap:normal;word-break:normal}.section-heading{display:block}.metrics{grid-template-columns:1fr}.metric-card{min-height:76px}.data-grid,.role-data-grid,.role-grid{grid-template-columns:1fr}.record-fields{grid-template-columns:1fr}.row{grid-template-columns:1fr}.row span,.record-actions{justify-content:flex-start}.role-actions button{width:100%}}\n`
}

function buildHtml({ domainLabel, deliveryLevel, screens, admin = false }) {
  const title = admin ? `${domainLabel} - Backoffice` : domainLabel
  const panels = screens.map((entry) => `<article class="workflow-card"><p class="eyebrow">Modulo</p><h3>${entry}</h3></article>`).join('')
  return `<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8">
...
<body data-admin="${admin ? 'true' : 'false'}">
  ...
  <main class="layout">
    ...
    <section class="panel"><div class="section-heading"><p>${admin ? 'Operacion interna' : 'Experiencia principal'}</p><h2>${admin ? 'Backoffice CRUD' : 'Superficie publica'}</h2></div><div class="role-grid">${panels}</div></section>
