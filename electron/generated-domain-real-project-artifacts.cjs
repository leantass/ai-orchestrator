...
function collectionPriority(collection) { const normalized = collection.toLocaleLowerCase(); const weights = [['order', 'pedido'], ['employee', 'empleado'], ['company', 'empresa'], ['menu'], ['production', 'produccion'], ['label', 'etiqueta'], ['report']]; const index = weights.findIndex((signals) => signals.some((signal) => normalized.includes(signal))); return index === -1 ? 99 : index }
...
async function renderSummary() { const dashboard = await api('/api/dashboard').then((result) => result.body); const { visible, remaining } = summaryEntries(dashboard, isAdmin ? 8 : 6); summary.innerHTML = visible.map(([collection, count]) => '<article class="metric-card"><strong>' + escapeHtml(count) + '</strong><span>' + escapeHtml(labelFor(collection)) + '</span></article>').join('') + (remaining > 0 ? '<article class="metric-card metric-card-muted"><strong>+' + escapeHtml(remaining) + '</strong><span>mas areas</span></article>' : '') }
...
function buildRoleHtml({ domainLabel, deliveryLevel, rolePage, rolePages }) {
  const nav = rolePages.map((page) => `<a class="${page.key === rolePage.key ? 'active' : ''}" href="/${page.path.replace(/^public\//u, '')}">${page.title}</a>`).join('')
...
function buildHtml({ domainLabel, deliveryLevel, screens, rolePages = [], admin = false }) {
  ...
  const panels = screens.map((entry, index) => `<article class="workflow-card surface-card"><span class="step-badge">${index + 1}</span><p class="eyebrow">Modulo</p><h3>${entry}</h3></article>`).join('')
  const roleLinks = !admin && rolePages.length > 0 ? `<section class="panel role-access-panel"><div class="section-heading"><p>Roles operativos</p><h2>Entradas para recorrer la demo</h2></div><div class="role-link-grid">${rolePages.map((page) => `<a class="role-link-card" href="/${page.path.replace(/^public\//u, '')}"><strong>${page.title}</strong><span>${page.subtitle}</span></a>`).join('')}</div></section>` : ''
  return `<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8">
...
<body data-admin="${admin ? 'true' : 'false'}">
  ...
  <main class="layout">
    <section class="hero"><p class="eyebrow">${deliveryLevel}</p><h1>${title}</h1><p>Demo local con SQLite, API REST, backoffice y flujos por rol listos para recorrer operaciones reales.</p><div id="summary" class="metrics">Cargando datos...</div></section>
    ${roleLinks}
