...
El alcance normativo de QA, seguridad y correction loop está documentado en [ORQUESTADOR_ESCALON_6_QA_SECURITY.md](ORQUESTADOR_ESCALON_6_QA_SECURITY.md). La implementación automatizada consume la salida aislada del Escalón 5 y produce evidencia local clasificada, findings sanitizados, gates persistidos, retornos explícitos y una superficie QA/IPC semántica; no conecta herramientas externas ni inicia Escalón 7. `ESCALON_6_STATUS=IMPLEMENTED_PENDING_HUMAN_GATE`.
